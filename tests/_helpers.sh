...
# Test our SSL image (Ignore SSL certificate with -k option).
function testssl {
    echo "=> Querying image SSL ($1)"
    curl --retry 10 --retry-delay 5 -k -o actual/$1.html https://localhost:$2 --stderr -
    checkstatus $?
